 ...
 */
package com.forgerock.sapi.gateway.ob.uk.rs.obie.api.account.v3_0.directdebits;
...
public interface DirectDebitsApi {
    ...
    @ApiOperation(value = "Get Direct Debits", notes = "Get Direct Debits", response = OBReadDirectDebit1.class, authorizations = {
            @Authorization(value = "PSUOAuth2Security", scopes = {
                    @AuthorizationScope(scope = "accounts", description = "Ability to get Accounts information")
            })
    })
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Direct Debits successfully retrieved", response = OBReadDirectDebit1.class),
            @ApiResponse(code = 400, message = "Bad Request", response = Void.class),
            @ApiResponse(code = 401, message = "Unauthorized", response = Void.class),
            @ApiResponse(code = 403, message = "Forbidden", response = Void.class),
            @ApiResponse(code = 405, message = "Method Not Allowed", response = Void.class),
    ...
    @RequestMapping(value = "/direct-debits",
            produces = {"application/json; charset=utf-8"},
            method = RequestMethod.GET)
    ResponseEntity<OBReadDirectDebit1> getDirectDebits(
            @ApiParam(value = "The unique id of the ASPSP to which the request is issued. " +
                    "The unique id will be issued by OB.", required = true)
            @RequestHeader(value = "x-fapi-financial-id", required = true) String xFapiFinancialId,
            ...
            @ApiParam(value = "The PSU's IP address if the PSU is currently logged in with the TPP.")
            @RequestHeader(value =
                    "x-fapi-customer-ip-address", required = false) String xFapiCustomerIpAddress,
