...
import com.forgerock.sapi.gateway.ob.uk.rs.server.common.util.link.v4_0_0.LinksHelper;
...
import uk.org.openbanking.datamodel.v4.payment.OBWritePaymentDetails1PaymentStatus;
import uk.org.openbanking.datamodel.v4.payment.OBWritePaymentDetails1PaymentStatusStatus;
import uk.org.openbanking.datamodel.v4.payment.OBWritePaymentDetails1PaymentStatusStatusDetail;
...
@Controller("DomesticStandingOrdersApiV4")
@RequestMapping("${openapi.paymentInitiation.base-path:/open-banking/v4.0/pisp}")
                ...
                .data(
                        new OBWritePaymentDetailsResponse1Data()
                                .addPaymentStatusItem(
                                        new OBWritePaymentDetails1PaymentStatus()
                                                .status(status)
                                                .paymentTransactionId(UUID.randomUUID().toString())
                                                .statusUpdateDateTime(new DateTime(frStandingOrderSubmission.getUpdated()))
                                                .statusDetail(
                                                        new OBWritePaymentDetails1PaymentStatusStatusDetail()
                                                                .status(status.getValue())
                                                                .statusReason(String.valueOf(statusReasonEnum))
                                                                .statusReasonDescription(statusReasonEnum.getValue())
                                                                .localInstrument(localInstrument.getValue())
                                                )
                                )
