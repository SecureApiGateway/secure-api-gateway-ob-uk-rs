...
import static com.forgerock.sapi.gateway.ob.uk.rs.server.common.util.v4_0_0.PaymentsUtils.createTestDataStandingOrderConsentResponse6;
...
import static uk.org.openbanking.testsupport.v4.payment.OBWriteDomesticStandingOrderConsentTestDataFactory.aValidOBWriteDomesticStandingOrder3DataInitiation;
...
import com.forgerock.sapi.gateway.ob.uk.rs.server.common.util.v4_0_0.PaymentsUtils;
...
public class DomesticStandingOrdersApiControllerTest {
    ...
    private static final String STANDING_ORDERS_URI = "/open-banking/v4.0.0/pisp/domestic-standing-orders";
    ...
    private void mockConsentStoreGetResponse(OBWriteDomesticStandingOrder3 paymentRequest, OBReadRefundAccount readRefundAccount) {
        mockConsentStoreGetResponse(paymentRequest, readRefundAccount, OBPaymentConsentStatus.AUTH.toString());
    }
    ...
    public void shouldGetDomesticStandingOrderPaymentDetailsById() {
        ...
        // Then
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
        @Valid List<@Valid OBWritePaymentDetails1> responseData = response.getBody().getData().getPaymentStatus();
        for (@Valid OBWritePaymentDetails1 data : responseData) {
    ...
    public void failsToCreatePaymentIfStatusNotAuthorised() {
        OBWriteDomesticStandingOrder3 paymentRequest = aValidOBWriteDomesticStandingOrder3();
        mockConsentStoreGetResponse(paymentRequest, OBReadRefundAccount.NO, OBPaymentConsentStatus.COND.toString());
        ...
        assertThat(errorResponse.getBody().getErrors().get(0)).isEqualTo(OBRIErrorType.CONSENT_STATUS_NOT_AUTHORISED.toOBError1(OBPaymentConsentStatus.COND.toString()));
