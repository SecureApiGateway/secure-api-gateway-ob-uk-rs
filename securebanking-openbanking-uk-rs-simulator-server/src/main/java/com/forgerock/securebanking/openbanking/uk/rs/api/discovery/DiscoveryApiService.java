    ...
    public Map<OBGroupName, Map<String, OBDiscoveryAPI>> getDiscoveryApis() {
        ...
        for (AvailableApiEndpoint availableEndpoint : availableEndpoints) {
            ...
            if (isVersionEnabled(version)
                ...
                log.error("Version is enabled for: {}, {}, {}", endpointUrl, availableEndpoint.getGroupName(), availableEndpoint.getApiReference());

                // Init map
                if (!discoveryApis.containsKey(availableEndpoint.getGroupName())) {
                    discoveryApis.put(availableEndpoint.getGroupName(), new HashMap<>());
                    log.error("Putting group in available endpoints: {}, {}, {}", endpointUrl, availableEndpoint.getGroupName(), availableEndpoint.getApiReference());
                }
                ...
                if (!discoveryApis.get(availableEndpoint.getGroupName()).containsKey(availableEndpoint.getVersion())) {
                    ...
                    log.error("Putting endpoints because no version was found for current endpoint: {}, {}, {}", endpointUrl, availableEndpoint.getGroupName(), availableEndpoint.getApiReference());
                }
                ...
                log.error("Version is not enabled for: {}, {}, {}", endpointUrl, availableEndpoint.getGroupName(), availableEndpoint.getApiReference());
            }
        }
