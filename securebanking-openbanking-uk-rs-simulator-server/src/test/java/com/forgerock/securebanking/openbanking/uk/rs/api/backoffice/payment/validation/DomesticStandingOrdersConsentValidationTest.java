...
/**
 * Unit test for {@link DomesticStandingOrdersConsentValidation}
 */
public class DomesticStandingOrdersConsentValidationTest {
    private PaymentConsentValidation validation;
    private OBWriteDomesticStandingOrderConsent4 consent4;

    @BeforeEach
    public void setup() {
        validation = new DomesticStandingOrdersConsentValidation();
        consent4 = aValidOBWriteDomesticStandingOrderConsent4();
    }

    private static Stream<Arguments> provideConsent() {
        return Stream.of(
                Arguments.arguments(aValidOBWriteDomesticStandingOrderConsent4()),
                Arguments.arguments(aValidOBWriteDomesticStandingOrderConsent5())
        );
    }
    ...
    public void shouldNotValidateAmountZERO() {
        String amount = "0";
        consent4.getData().getInitiation().getFirstPaymentAmount().setAmount(amount);
    ...
    public void shouldNotValidateCurrency() {
        ...
        consent4.getData().getInitiation().getFirstPaymentAmount().setCurrency(currency);
        validation.clearErrors().validate(consent4);
        assertThat(validation.getErrors()).isNotEmpty().containsExactly(
                OBRIErrorType.DATA_INVALID_REQUEST.toOBError1(
